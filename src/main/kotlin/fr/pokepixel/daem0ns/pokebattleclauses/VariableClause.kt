...
import fr.pokepixel.daem0ns.pokebattleclauses.config.ClausesConfig.ClauseConfig
...
class VariableClause(id: String, private val clauseConfig: ClauseConfig) : BattleClause(id) {
    ...
    override fun validateSingle(pokemon: Pokemon): Boolean {
        ...
        val debugEnabled = PokeBattleClauses.configAccessors.general.get().debug

        fun debug(message: String) {
            if (debugEnabled) PokeBattleClauses.logger.info("{Clause '$id' debug ${pokemon.displayName}} $message")
        }
        ...
        val pokeCheckPassed = clauseConfig.pokemons?.let { pokeConfig ->
            if(pokemon.formEnum.formSuffix=="" || pokemon.formEnum==EnumNoForm.NoForm  || pokemon.form==0 || pokemon.form==-1){
                ...
                if (pokeConfig.ensureInitialization()) {
                    val pokeInList = pokeConfig.listValues!!.any { poke -> pokemon.species.pokemonName.replace(" ","") == poke}
                    pokeAllowed = when (pokeConfig.listType) {
                        WHITE -> pokeInList
                        BLACK -> !pokeInList
                    }
                }
                debug("Pokemon '${pokemon.species.pokemonName }' allowed: $pokeAllowed")
